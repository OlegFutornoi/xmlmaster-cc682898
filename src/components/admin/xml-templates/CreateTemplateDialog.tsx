...
const CreateTemplateDialog = ({ open, onOpenChange, onCreateTemplate, isCreating }: CreateTemplateDialogProps) => {
  ...
  const parsePriceFormat = (xmlDoc: Document, structure: ParsedXMLStructure): ParsedXMLStructure => {
    ...
    if (!priceElement) {
      throw new Error('Не знайдено кореневий елемент <price>');
    }
    ...
    // Парсинг валют
    const currencies = priceElement.querySelectorAll('currency');
    if (currencies.length > 0) {
      structure.currencies = Array.from(currencies).map(currency => ({
        id: currency.getAttribute('id') || '',
        rate: parseFloat(currency.getAttribute('rate') || '1')
    ...
    // Парсинг категорій
    const categories = priceElement.querySelectorAll('category');
    if (categories.length > 0) {
      structure.categories = Array.from(categories).map(category => ({
        id: category.getAttribute('id') || '',
        name: category.textContent || ''
      ...
      if (firstCategory) {
        structure.parameters.push({
          name: 'category_id',
          value: firstCategory.getAttribute('id') || '',
          path: '/price/categories/category[@id]',
          type: 'parameter',
          category: 'category'
        });
        structure.parameters.push({
          name: 'category_name',
          value: firstCategory.textContent || '',
          path: '/price/categories/category',
          type: 'parameter',
          category: 'category'
        });
      }
    ...
    // Парсинг товарів (items)
    const items = priceElement.querySelectorAll('item');
    if (items.length > 0) {
        ...
        if (params.length > 0) {
          ...
          if (paramName) {
            structure.parameters.push({
              name: 'param_name',
              value: paramName,
              path: '/price/items/item/param[@name]',
              type: 'characteristic',
              category: 'offer'
            });
            structure.parameters.push({
