...
export const PlanLimitationsList = ({
  selectedPlanId,
  planLimitations: propsPlanLimitations
}: PlanLimitationsListProps) => {
  // Використовуємо хук лише якщо передано selectedPlanId
  const {
    planLimitations: hookPlanLimitations,
    updateLimitationValue
  } = selectedPlanId ? usePlanLimitations(selectedPlanId) : {
    planLimitations: [],
    updateLimitationValue: null
  };
  ...
  return <div className="space-y-2" id="plan-limitations-list">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {limitations.map(limitation => <div key={limitation.id} className="flex items-center justify-between p-2 border rounded-md" id={`limitation-${limitation.id}`}>
            ...
            {updateLimitationValue && editingLimitation === limitation.id ? <div className="flex items-center space-x-2">
                <Input type="number" value={editValue} onChange={e => setEditValue(e.target.value)} className="w-20 h-8" min={0} id={`limitation-value-input-${limitation.id}`} />
                <Button variant="ghost" size="sm" onClick={() => handleSaveEdit(limitation.id)} id={`save-limitation-${limitation.id}`}>
                  <Check className="h-4 w-4 text-green-500" />
                </Button>
                <Button variant="ghost" size="sm" onClick={handleCancelEdit} id={`cancel-limitation-${limitation.id}`}>
                  <X className="h-4 w-4 text-red-500" />
                </Button>
              </div> : <div className="flex items-center space-x-2">
                <span className="font-semibold">{limitation.value}</span>
                {updateLimitationValue && <Button variant="ghost" size="sm" onClick={() => handleStartEdit(limitation)} id={`edit-limitation-${limitation.id}`}>
