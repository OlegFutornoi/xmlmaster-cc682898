...
import { ShoppingBag, Upload, Check, X, Plus, Tag, Image as ImageIcon, Edit, Eye, Trash, ChevronDown, ChevronUp, ArrowLeft, ArrowRight, ExternalLink, Save, Undo2 } from 'lucide-react';
...
const UserProducts = () => {
  ...
  } = usePlanLimitations(activeSubscription?.tariff_plan?.id || null);
  ...
  const fetchUserSuppliers = async () => {
    try {
      const {
        data,
        error
      } = await extendedSupabase.from('suppliers').select('*').eq('user_id', user?.id).order('created_at', {
        ascending: false
      });
      if (error) throw error;
      setSuppliers(data || []);
  ...
  const fetchStoreProductsCount = async () => {
    ...
    try {
      const {
        ...
        error,
        count
      } = await extendedSupabase.from('products').select('*', {
        count: 'exact'
  ...
  const parseXml = async (xmlText: string) => {
    try {
      ...
      const categoryMap: {
        [key: string]: string;
      } = {}; // Для зберігання відповідності id -> name
  ...
  const handleCategorySelect = (categoryId: string) => {
    ...
    if (updatedCategories) {
      ...
      const newSelectedCategories = updatedCategories.filter(category => category.selected).map(category => category.id);
  ...
  const handleProductSelect = (productId: string) => {
    ...
    if (updatedProducts) {
      ...
      const newSelectedProducts = updatedProducts.filter(product => product.selected).map(product => product.id);
      setSelectedProducts(newSelectedProducts);
    }
  };
  ...
  const selectAllInCategory = (categoryId: string) => {
    ...
    if (updatedCategories) {
      ...
      // Оновлюємо список вибраних продуктів
      const newSelectedProducts = allProducts.filter(product => product.selected).map(product => product.id);
      setSelectedProducts(newSelectedProducts);
    }
  };
    ...
    try {
      ...
      for (const product of previewProducts) {
        ...
        const {
          ...
          error: categoryError
        } = await extendedSupabase.from('product_categories').select('id').eq('name', categoryName).eq('store_id', selectedStore).eq('user_id', user?.id);
        ...
        if (!existingCategories?.length) {
          ...
          const {
            data: newCategory,
            error
          } = await extendedSupabase.from('product_categories').insert({
            name: categoryName,
            user_id: user?.id,
        ...
        const {
          data: savedProduct,
          error: productError
        } = await extendedSupabase.from('products').insert({
          name: product.name,
        ...
        if (product.images?.length) {
          ...
          const {
            error: imagesError
          } = await extendedSupabase.from('product_images').insert(productImages);
          if (imagesError) {
        ...
        if (product.attributes?.length) {
          ...
          } = await extendedSupabase.from('product_attributes').insert(productAttributes);
          if (attributesError) {
      ...
      const {
        error: supplierError
      } = await extendedSupabase.from('suppliers').update({
        product_count: previewProducts.length
  ...
  const filteredCategories = parsedData?.categories.filter(category => category.name.toLowerCase().includes(searchQuery.toLowerCase()) && (!hideEmptyCategories || category.products.length > 0)) || [];

  // Фільтрація продуктів за пошуком та категоріями
  const filteredProducts = parsedData?.products.filter(product => {
    const matchesSearch = product.name.toLowerCase().includes(searchQuery.toLowerCase()) || product.description && product.description.toLowerCase().includes(searchQuery.toLowerCase()) || product.vendor && product.vendor.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return <div className="container mx-auto py-4 px-4">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {productsLimit > 0 && <Badge variant="outline" className="flex items-center px-2 py-1 text-xs">
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="outline" onClick={() => fileInputRef.current?.click()} disabled={!selectedStore || !selectedSupplier} id="upload-xml-button" size="sm">
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="secondary" onClick={loadFileFromUrl} disabled={!selectedStore || !selectedSupplier || !suppliers.find(s => s.id === selectedSupplier)?.url} id="load-from-url-button" size="sm">
          ...
          <input type="file" accept=".xml" onChange={handleFileUpload} ref={fileInputRef} className="hidden" />
        </div>
      </div>
      ...
      {isLoading ? <div className="flex justify-center items-center py-12">
          ...
          <CardContent>
            <Button onClick={() => navigate('/user/dashboard/stores')} variant="secondary">
              <ShoppingBag className="h-4 w-4 mr-2" />
              Перейти до сторінки магазинів
            </Button>
          </CardContent>
        </Card> : suppliers.length === 0 ? <Card>
          <CardHeader>
          ...
          <CardContent>
            <Button onClick={() => navigate('/user/dashboard/suppliers')} variant="secondary">
              <Tag className="h-4 w-4 mr-2" />
              Перейти до сторінки постачальників
            </Button>
          </CardContent>
        </Card> : !selectedStore || !selectedSupplier ? <div className="text-center py-8">
          <p>Будь ласка, виберіть магазин і постачальника для продовження</p>
        </div> : !canAddProducts ? <Card>
          <CardHeader>
          ...
          <CardContent>
            <div className="flex flex-col items-center p-4 border rounded bg-red-50">
              ...
              <Button variant="outline" className="mt-4" onClick={() => navigate('/user/dashboard/tariffs')}>
          ...
          <CardContent className="pt-4">
            ...
            <div className="mb-4 flex flex-col md:flex-row gap-2 items-center">
              <div className="flex items-center gap-2 w-full md:w-auto">
                <Input placeholder="Пошук за назвою товару чи категорії..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="flex-1" id="search-input" />
              ...
              <div className="flex items-center gap-2 ml-auto">
                <div className="flex items-center space-x-2">
                  <Checkbox id="hide-empty" checked={hideEmptyCategories} onCheckedChange={checked => {
                setHideEmptyCategories(checked as boolean);
              }} />
                  <label htmlFor="hide-empty" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
            ...
            <div className="flex justify-between items-center mb-2 text-sm font-medium">
              ...
              <div>
                <Button onClick={showProductsPreview} disabled={selectedProducts.length === 0 || selectedProducts.length > remainingProductsLimit} size="sm" variant="default" id="preview-products-button">
                  <Eye className="h-4 w-4 mr-2" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 overflow-hidden">
              ...
              <div className="border rounded-md p-2 overflow-y-auto max-h-[calc(100vh-320px)]">
                ...
                {filteredCategories.length > 0 ? <div className="space-y-2">
                    {filteredCategories.map(category => <div key={category.id} className={`p-2 rounded-md cursor-pointer ${selectedCategories.includes(category.id) ? 'bg-blue-50 border border-blue-200' : 'hover:bg-gray-50 border border-transparent'}`} onClick={() => handleCategorySelect(category.id)}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center">
                            <Checkbox checked={category.selected} id={`category-${category.id}`} />
                            <label htmlFor={`category-${category.id}`} className="ml-2 font-medium text-sm cursor-pointer flex-1">
                              {category.name}
                            </label>
                          </div>
                          <Badge variant="outline">
                  ...
                  </div> : <p className="text-center py-4 text-gray-500">Немає категорій, що відповідають пошуку</p>}
              </div>
              ...
              <div className="border rounded-md p-2 overflow-hidden md:col-span-2">
                ...
                {filteredProducts.length > 0 ? <div className="overflow-y-auto max-h-[calc(100vh-330px)]">
                    <Table>
                      ...
                      <TableBody>
                        {filteredProducts.map(product => <TableRow key={product.id} className={product.selected ? 'bg-blue-50' : ''}>
                            <TableCell className="p-2">
                              <Checkbox checked={product.selected} id={`product-${product.id}`} onCheckedChange={() => handleProductSelect(product.id)} />
                            </TableCell>
                            <TableCell className="p-2">
                              {product.images.length > 0 ? <div className="w-12 h-12 rounded overflow-hidden border">
                                  <img src={product.images[0]} alt={product.name} className="w-full h-full object-contain" onError={e => {
                          (e.target as HTMLImageElement).src = '/placeholder.svg';
                        }} />
                                </div> : <div className="w-12 h-12 bg-gray-100 rounded flex items-center justify-center">
                            ...
                            <TableCell className="hidden md:table-cell text-right">
                              {product.stock_quantity !== undefined ? <Badge variant={product.stock_quantity > 0 ? "outline" : "secondary"}>
                  ...
                  </div> : <div className="flex flex-col items-center justify-center h-[calc(100vh-350px)]">
      ...
      if (!isSaving) {
        ...
        <DialogContent className="overflow-y-auto max-w-4xl max-h-[90vh]">
          ...
          {currentProduct && <div className="flex flex-col h-full overflow-hidden">
              {/* Товар для перегляду */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                ...
                <div className="flex flex-col">
                  <div className="relative aspect-square bg-gray-100 rounded-lg overflow-hidden border mb-4 max-h-[300px]">
                    {currentProduct.images && currentProduct.images.length > 0 ? <Carousel className="w-full">
                        <CarouselContent>
                          {currentProduct.images.map((imageUrl, idx) => <CarouselItem key={idx} className="flex justify-center items-center">
                              <img src={imageUrl} alt={`${currentProduct.name} - зображення ${idx + 1}`} className="object-contain w-full h-full max-h-[300px]" onError={e => {
                        (e.target as HTMLImageElement).src = '/placeholder.svg';
                      }} />
                            </CarouselItem>)}
                        </CarouselContent>
                      ...
                      </Carousel> : <div className="flex justify-center items-center h-full">
                  ...
                  {/* Мініатюри зображень */}
                  {currentProduct.images && currentProduct.images.length > 1 && <div className="flex flex-wrap gap-2">
                      {currentProduct.images.map((imageUrl, idx) => <div key={idx} className="w-16 h-16 border rounded overflow-hidden">
                          <img src={imageUrl} alt={`Мініатюра ${idx + 1}`} className="w-full h-full object-cover" onError={e => {
                    (e.target as HTMLImageElement).src = '/placeholder.svg';
                  }} />
                        </div>)}
                    </div>}
                ...
                <div className="flex flex-col">
                  ...
                  {/* Виробник */}
                  {currentProduct.vendor && <div className="flex items-center mb-3">
                  ...
                  <div className="flex flex-col mb-4">
                    ...
                    {currentProduct.old_price && currentProduct.old_price > currentProduct.price && <div className="flex items-center">
                    ...
                    {currentProduct.sale_price && currentProduct.sale_price < currentProduct.price && <div className="flex items-center">
                  ...
                  <div className="flex items-center mb-4">
                    ...
                    <Badge variant={currentProduct.stock_quantity && currentProduct.stock_quantity > 0 ? "success" : "secondary"}>
                      {currentProduct.stock_quantity && currentProduct.stock_quantity > 0 ? `В наявності (${currentProduct.stock_quantity} шт.)` : "Немає в наявності"}
                    </Badge>
                  </div>
                  
                  {/* Опис товару */}
                  {currentProduct.description && <div className="mb-4">
                      <h3 className="text-md font-medium mb-2">Опис</h3>
                      <div className="text-sm text-gray-700 bg-gray-50 p-3 rounded overflow-y-auto max-h-[100px]" dangerouslySetInnerHTML={{
                  __html: currentProduct.description
                }}></div>
                    </div>}
                  
                  {/* Характеристики товару */}
                  {currentProduct.attributes && currentProduct.attributes.length > 0 && <div>
                      <h3 className="text-md font-medium mb-2">Характеристики</h3>
                      <ScrollArea className="border rounded max-h-[150px]">
                        <div className="p-3">
                          <table className="w-full">
                            <tbody>
                              {currentProduct.attributes.map((attr, idx) => <tr key={idx} className={idx % 2 === 0 ? "bg-gray-50" : "bg-white"}>
          ...
          <DialogFooter className="flex justify-between items-center mt-4">
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="outline" onClick={() => setIsPreviewMode(false)} disabled={isSaving}>
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="icon" onClick={goToPrevProduct} disabled={currentPreviewIndex === 0 || isSaving}>
              ...
              <Button variant="outline" size="icon" onClick={goToNextProduct} disabled={currentPreviewIndex === previewProducts.length - 1 || isSaving}>
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="default" onClick={saveSelectedProducts} disabled={isSaving}>
                    {isSaving ? <>
