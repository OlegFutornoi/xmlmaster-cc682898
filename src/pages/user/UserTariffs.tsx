...
const UserTariffs = () => {
  ...
  if (subscriptionsLoading || plansLoading) {
    return <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 flex items-center justify-center">
      ...
      <div className="bg-white/80 backdrop-blur-sm border-b border-emerald-100 px-6 py-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            {subscriptionHistory.length > 0 && <div className="flex items-center gap-3 mt-4 md:mt-0">
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="outline" size="icon" onClick={() => setIsHistoryModalOpen(true)} className="border-emerald-200 hover:bg-emerald-50 hover:border-emerald-300" id="history-button">
      ...
      <div className={`max-w-7xl mx-auto ${isMobile ? 'px-4 py-6' : 'px-6 py-8'}`} id="user-tariffs-container">
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {tariffPlans.map(plan => <TariffCard key={plan.id} plan={plan} isActive={activeSubscription?.tariff_plan.id === plan.id} onSelect={handlePlanClick} onViewDetails={handleViewDetails} />)}
          </div>
        </div>
        ...
        <SubscriptionHistoryModal open={isHistoryModalOpen} onOpenChange={setIsHistoryModalOpen} history={subscriptionHistory} />
      </div>
    </div>;
};
