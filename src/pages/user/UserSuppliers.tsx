...
const UserSuppliers = () => {
  ...
  const fetchUserLimitations = async () => {
    ...
    try {
      ...
      } = await extendedSupabase.from('tariff_plan_limitations').select(`
          value,
          limitation_types:limitation_type_id (name, description)
        `).eq('tariff_plan_id', activeSubscription.tariff_plan.id).eq('limitation_types.name', 'suppliers_count');
      if (limitationError) {
  ...
  const handleCreateSupplier = async () => {
    ...
    try {
      const {
        data,
        error
      } = await extendedSupabase.from('suppliers').insert({
        user_id: user.id,
  ...
  const handleDeleteSupplier = async () => {
    ...
    try {
      const {
        error
      } = await supabase.from('suppliers').delete().eq('id', supplierToDelete.id).eq('user_id', user.id);
      if (error) {
        throw error;
      }
      toast({
  ...
  return <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-3 mt-4 md:mt-0">
              {suppliersLimit !== null && <Badge variant="outline" className="bg-emerald-50 border-emerald-200 text-emerald-700 px-3 py-1">
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button disabled={!canCreateSupplier} onClick={resetDialog} className="bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white border-0 shadow-lg" id="create-supplier-button">
                      <PlusCircle className="h-4 w-4 mr-2" />
                      Додати постачальника
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>
                    {canCreateSupplier ? "Додати постачальника" : `Досягнуто ліміт постачальників (${suppliersLimit})`}
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
      ...
      <div className="max-w-7xl mx-auto p-6">
        {isLoading ? <div className="text-center py-12">
          ...
          </div> : suppliers.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {suppliers.map(supplier => <Card key={supplier.id} className="bg-white/80 backdrop-blur-sm border-emerald-100 shadow-lg hover:shadow-xl transition-all duration-300 group relative overflow-hidden">
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex gap-1 relative z-20">
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button variant="ghost" size="icon" onClick={e => openEditDialog(e, supplier)} className="h-8 w-8 text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 relative z-30" id={`edit-supplier-${supplier.id}`} type="button">
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button variant="ghost" size="icon" onClick={e => openDeleteDialog(e, supplier)} className="h-8 w-8 text-red-500 hover:text-red-700 hover:bg-red-50 relative z-30" id={`delete-supplier-${supplier.id}`} type="button">
                ...
                <CardContent className="pt-0 relative z-10">
                  {supplier.url ? <a href={supplier.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-emerald-600 hover:text-emerald-700 text-sm font-medium relative z-20" onClick={e => e.stopPropagation()}>
                      <ExternalLink className="h-4 w-4" />
                      Відкрити URL
                    </a> : <span className="text-gray-500 text-sm">URL не вказано</span>}
                </CardContent>
              </Card>)}
          </div> : <Card className="bg-white/80 backdrop-blur-sm border-emerald-100 shadow-lg max-w-2xl mx-auto">
            ...
            <CardContent className="text-center">
              {canCreateSupplier ? <Button onClick={resetDialog} className="bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white border-0 shadow-lg">
        ...
        <DialogContent className="bg-white/95 backdrop-blur-sm border-emerald-100">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input id="supplier-name" value={supplierName} onChange={e => setSupplierName(e.target.value)} placeholder="Введіть назву постачальника" className="border-emerald-200 focus:border-emerald-400 focus:ring-emerald-400" />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="supplier-url" className="text-gray-700">URL (не обов'язково)</Label>
              <Input id="supplier-url" value={supplierUrl} onChange={e => setSupplierUrl(e.target.value)} placeholder="Введіть URL постачальника" className="border-emerald-200 focus:border-emerald-400 focus:ring-emerald-400" />
          ...
          <DialogFooter>
            ...
            <Button onClick={isEditMode ? handleEditSupplier : handleCreateSupplier} disabled={isSubmitting} className="bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white border-0">
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent className="bg-white/95 backdrop-blur-sm border-emerald-100">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeleteSupplier} disabled={isDeleting} className="bg-red-500 hover:bg-red-600 text-white">
